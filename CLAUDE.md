...
skill. It also holds `research/ideas/`: proposals from `kill-video-concepts`, plus an
append-only `ledger.log` recording every concept the gauntlet judged and the gate that killed
it. `DEAD` is never re-proposed; `DEFERRED` carries a query-decidable `reopen=` condition and
comes back once that condition passes — so the ledger is an active index, not a graveyard.
Nothing in `sources/` reads it — it is human-facing only.
