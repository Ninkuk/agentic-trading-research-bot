...
def test_run_wasde_stores_discovered_release(tmp_path):
    ...
    sid, ncommod, nobs = runmod.run_wasde(
        db_path,
        now_iso=NOW,
        fetch=lambda: [_obs("Corn", "ending_stocks", 2029.0), _obs("Corn", "total_use", 16280.0)],
    )
    assert nobs == 2 and ncommod == 1
    conn = sqlite3.connect(db_path)
    r = conn.execute(
        "SELECT ROUND(stocks_to_use,3) FROM v_wasde_stocks_to_use WHERE commodity='Corn'"
    ).fetchone()
    assert r[0] == round(2029.0 / 16280.0, 3)


def test_run_wasde_no_release_found_writes_zero(tmp_path, capsys):
    sid, ncommod, nobs = runmod.run_wasde(str(tmp_path / "w.db"), fetch=lambda: None, now_iso=NOW)
