...
def test_next_early_close_returns_next_on_or_after():
    conn = _fresh()
    monitor_common.upsert_events(conn, [_evt("early_close", "2026-11-27", "13:00")], "t")
...
def test_v_upcoming_closures_lists_holidays_and_early_closes_from_today():
    ...
    dates = [
        r[0] for r in conn.execute("SELECT event_date FROM v_upcoming_closures ORDER BY event_date")
    ]
...
def test_v_next_opex_returns_soonest_expiration():
    ...
    row = conn.execute("SELECT event_date, event_type FROM v_next_opex").fetchone()
