        ...
        out.append(
            {
                ...
                "total_volume": tv,
                "short_ratio": (sv / tv if tv else None),
                "market": "Q",
            }
        )
    return out
...
def test_replace_day_replaces_and_drops_removed_rows():
    ...
    assert [tuple(r) for r in conn.execute("SELECT symbol, short_volume FROM short_volume")] == [
        ("AA", 150)
    ]
...
def test_upsert_securities_tracks_first_and_last_seen():
    ...
    row = conn.execute("SELECT first_seen, last_seen FROM securities WHERE symbol='AA'").fetchone()
...
def test_prune_removes_old_snapshots_only():
    ...
    assert conn.execute("SELECT COUNT(*) FROM short_volume").fetchone()[0] == 1  # untouched
