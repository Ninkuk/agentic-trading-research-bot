...
def _seed():
    ...
    db.upsert_companies(conn, [{"cik": 1, "ticker": "AAA", "name": "Alpha", "sic": "1"}], "t")
    return conn
...
def test_v_latest_fundamentals_picks_newest_period():
    ...
    row = conn.execute("SELECT value FROM v_latest_fundamentals WHERE tag='Assets'").fetchone()
    assert row == (150.0,)
...
def test_v_screener_derives_ratios():
    ...
    r = conn.execute(
        "SELECT net_margin, roe, debt_to_equity FROM v_screener WHERE cik=1"
    ).fetchone()
...
def test_v_screener_null_ratio_when_denominator_absent():
    conn = _seed()
    db.write_facts(conn, 1, [_f("NetIncomeLoss", "2024-12-31", "10-K", 200, "2025-02-01")])
...
def test_v_frame_cross_section_returns_all_filers_for_tag_period():
    conn = _seed()
    db.upsert_companies(conn, [{"cik": 2, "ticker": "BBB", "name": "Beta", "sic": "1"}], "t")
    ...
    rows = conn.execute(
        ...
        "WHERE tag='Assets' AND period_end='2024-12-31' ORDER BY value"
    ).fetchall()
...
def test_v_revisions_surfaces_restatement_with_delta():
    ...
    rows = conn.execute(
        "SELECT form, value, value_delta FROM v_revisions WHERE tag='NetIncomeLoss' ORDER BY filed"
    ).fetchall()
