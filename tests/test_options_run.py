...
from sources.screeners.cboe_options import run as run_mod
...
def _payload(underlying, iv30=27.8):
    return {
        ...
        "data": {
            ...
            "close": 100.5,
            "iv30": iv30,
            "last_trade_time": "2026-07-02T16:00:00",
            "options": [
                {
                    "option": f"{underlying}260717C00100000",
                    "bid": 1.0,
                    "ask": 1.2,
                    "iv": 0.3,
                    "delta": 0.5,
...
def test_run_ingests_symbols(tmp_path):
    ...
    sid, sc, rc = run_mod.run(
        dbp, symbols=["AAPL", "MSFT"], now_iso=NOW, fetch_chain=lambda sym, is_index: _payload(sym)
    )
    assert (sc, rc) == (2, 2)
    conn = db.connect(dbp)
    ...
    assert (
        conn.execute("SELECT DISTINCT snapshot_date FROM option_snapshots").fetchone()[0]
        == "2026-07-02"
    )
...
def test_run_skips_none_payload(tmp_path):
    ...
    _, sc, rc = run_mod.run(dbp, symbols=["AAPL", "ZZZZ"], now_iso=NOW, fetch_chain=fc)
    assert (sc, rc) == (1, 1)
...
def test_run_skips_failing_symbol_and_hides_message(tmp_path, capsys):
    ...
    _, sc, _ = run_mod.run(dbp, symbols=["BAD", "AAPL"], now_iso=NOW, fetch_chain=fc)
    assert sc == 1
...
def test_run_all_fail_writes_zero_snapshot(tmp_path):
    dbp = str(tmp_path / "opt.db")
    _, sc, rc = run_mod.run(dbp, symbols=["A"], now_iso=NOW, fetch_chain=lambda sym, is_index: None)
    assert (sc, rc) == (0, 0)
    conn = db.connect(dbp)
    assert tuple(conn.execute("SELECT symbol_count, row_count FROM snapshots").fetchone()) == (0, 0)
...
def test_run_keep_days_prunes_headers(tmp_path):
    dbp = str(tmp_path / "opt.db")
    run_mod.run(dbp, symbols=["A"], now_iso=NOW, fetch_chain=lambda sym, is_index: None)
    conn = db.connect(dbp)
    db.write_snapshot(conn, "2000-01-01T00:00:00+00:00", 0, 0)
    conn.close()
    run_mod.run(
        dbp, symbols=["A"], now_iso=NOW, keep_days=30, fetch_chain=lambda sym, is_index: None
    )
    conn = db.connect(dbp)
    old = conn.execute(
