...
def test_mark_palette_is_the_validated_set():
    ...
    # gray by rule; single-series lines take categorical slot-1 blue. Changing
...
def test_style_uses_mark_tokens():
    ...
    assert f"--mark-mid:{style.MARK_MID}" in style._STYLE
    assert f"--mark-line:{style.MARK_LINE}" in style._STYLE
