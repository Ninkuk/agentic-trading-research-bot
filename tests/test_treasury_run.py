...
def _raw_debt(dates):
    return [
        {
            "record_date": d,
            "tot_pub_debt_out_amt": "100",
            "debt_held_public_amt": "70",
            "intragov_hold_amt": "30",
        }
        for d in dates
    ]
...
def test_run_happy_path_counts_rows(tmp_path):
    ...
    sid, nds, nrows = runmod.run(
        db_path,
        only=["debt_penny", "yield_curve"],
        fetch_dataset=fetch_dataset,
        fetch_yield_curve=fetch_yc,
        now_iso=NOW,
    )
...
def test_run_skips_failing_dataset_without_leaking_secret(tmp_path, capsys):
    ...
    sid, nds, nrows = runmod.run(
        str(tmp_path / "t.db"), only=["debt_penny"], fetch_dataset=fetch_dataset, now_iso=NOW
    )
...
def test_run_all_fail_writes_zero_snapshot(tmp_path, capsys):
    ...
    sid, nds, nrows = runmod.run(
        str(tmp_path / "t.db"), only=["debt_penny"], fetch_dataset=boom, now_iso=NOW
    )
...
def test_run_incremental_reabsorbs_restated_prior_day(tmp_path):
    ...
    conn = sqlite3.connect(db_path)
    val = conn.execute(
        "SELECT tot_pub_debt_out FROM debt_penny WHERE record_date='2026-06-28'"
    ).fetchone()[0]
...
def test_run_full_ignores_incremental_lookback(tmp_path):
    ...
    runmod.run(db_path, only=["debt_penny"], fetch_dataset=fetch_dataset, now_iso=NOW)
    runmod.run(db_path, only=["debt_penny"], fetch_dataset=fetch_dataset, full=True, now_iso=NOW)
