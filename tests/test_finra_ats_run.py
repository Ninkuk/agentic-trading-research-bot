...
def test_run_delay_aware_end_and_counts(tmp_path):
    ...
    sid, wc, rc = runmod.run(db_path, start="2026-06-01", fetch_week=fetch_week, now_iso=NOW)
...
def test_run_incremental_skips_stored_but_refetches_trailing(tmp_path):
    db_path = str(tmp_path / "a.db")
    runmod.run(db_path, start="2026-06-01", fetch_week=lambda w: [_row(w)], now_iso=NOW)
    second = []
...
def test_run_keep_days_prunes_snapshots_not_facts(tmp_path):
    ...
    runmod.run(
        db_path,
        start="2026-06-01",
        fetch_week=lambda w: [_row(w)],
        now_iso="2026-01-01T00:00:00+00:00",
    )
    runmod.run(
        db_path, start="2026-06-01", fetch_week=lambda w: [_row(w)], now_iso=NOW, keep_days=30
    )
    conn = sqlite3.connect(db_path)
