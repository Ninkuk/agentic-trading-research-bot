...
def test_run_happy_path_counts(tmp_path):
    ...
    def fetch_domain(endpoint, *, start=None, end=None):
        if "rates" in endpoint:
            return [{"effectiveDate": "2026-06-01", "type": "SOFR", "percentRate": "5.3"}]
    ...
    sid, nd, nr = runmod.run(
        db_path, only=["reference_rates", "rrp"], fetch_domain=fetch_domain, now_iso=NOW
    )
...
def test_run_all_fail_zero_snapshot(tmp_path, capsys):
    ...
    sid, nd, nr = runmod.run(
        str(tmp_path / "n.db"), only=["reference_rates"], fetch_domain=boom, now_iso=NOW
    )
    assert (nd, nr) == (0, 0)
...
def test_run_incremental_floors_start_at_max_date(tmp_path):
    ...
    def fetch_domain(endpoint, *, start=None, end=None):
        ...
        return [{"effectiveDate": "2026-06-02", "type": "SOFR", "percentRate": "5.3"}]

    runmod.run(db_path, only=["reference_rates"], fetch_domain=fetch_domain, now_iso=NOW)
    runmod.run(db_path, only=["reference_rates"], fetch_domain=fetch_domain, now_iso=NOW)
...
def test_run_incremental_reabsorbs_restated_prior_day(tmp_path):
    ...
    conn = sqlite3.connect(db_path)
    val = conn.execute(
        "SELECT percent_rate FROM reference_rates WHERE effective_date='2026-05-30'"
    ).fetchone()[0]
...
def test_run_full_ignores_incremental_lookback(tmp_path):
    ...
    runmod.run(db_path, only=["reference_rates"], fetch_domain=fetch_domain, now_iso=NOW)
    runmod.run(db_path, only=["reference_rates"], fetch_domain=fetch_domain, full=True, now_iso=NOW)
...
def test_run_keep_days_prunes_snapshots_not_facts(tmp_path):
    ...
    runmod.run(
        db_path,
        only=["reference_rates"],
        fetch_domain=fetch_domain,
        now_iso="2026-01-01T00:00:00+00:00",
    )
    runmod.run(
        db_path, only=["reference_rates"], fetch_domain=fetch_domain, now_iso=NOW, keep_days=30
    )
    conn = sqlite3.connect(db_path)
    assert conn.execute("SELECT COUNT(*) FROM snapshots").fetchone()[0] == 1
    assert conn.execute("SELECT COUNT(*) FROM reference_rates").fetchone()[0] == 1
