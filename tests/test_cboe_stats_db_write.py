...
def test_write_vix_column_merge_does_not_blank_siblings():
    ...
    db.write_vix(
        conn,
        "VIX",
        [{"date": "2026-06-01", "open": 14.0, "high": 15.0, "low": 13.5, "close": 14.6}],
    )
    db.write_vix(
        conn,
        "VIX3M",
        [{"date": "2026-06-01", "open": None, "high": None, "low": None, "close": 16.2}],
    )
    row = conn.execute("SELECT close, vix3m FROM vix_daily WHERE date='2026-06-01'").fetchone()
...
def test_write_vix_reupsert_overwrites_in_place():
    ...
    db.write_vix(
        conn, "VIX", [{"date": "2026-06-01", "open": 1, "high": 1, "low": 1, "close": 15.0}]
    )
