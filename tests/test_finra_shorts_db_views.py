...
def _insert(conn, rows):
    ...
    conn.executemany(
        f"INSERT INTO short_volume ({','.join(_COLS)}) VALUES ({','.join(':' + c for c in _COLS)})",
        rows,
    )
    conn.commit()
...
def test_v_latest_only_max_date_and_liquid():
    conn = _fresh()
    _insert(conn, _rows("A", [("2024-06-13", 0.4, 200000), ("2024-06-14", 0.6, 200000)]))
    _insert(conn, _rows("B", [("2024-06-14", 0.9, 50000)]))  # illiquid -> excluded
    rows = conn.execute("SELECT symbol, date, short_ratio FROM v_latest").fetchall()
...
def test_v_ratio_spikes_against_trailing_average():
    ...
    ratio, base, spike = conn.execute(
        "SELECT short_ratio, base, spike_ratio FROM v_ratio_spikes WHERE symbol='A'"
    ).fetchone()
    assert ratio == pytest.approx(0.6)
...
def test_v_short_streaks_below_threshold_day_splits_run():
    ...
    streaks = sorted(
        (r[0], r[1])
        for r in conn.execute("SELECT streak_days, active FROM v_short_streaks WHERE symbol='A'")
    )
