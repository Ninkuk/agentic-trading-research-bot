...
def test_run_upserts_then_firms_status_in_place(tmp_path):
    ...
    rows = conn.execute("SELECT status FROM events WHERE event_type='fomc_meeting'").fetchall()
...
def test_run_replace_forward_drops_cancelled_future_meeting(tmp_path):
    ...
    conn = sqlite3.connect(db_path)
    n = conn.execute("SELECT COUNT(*) FROM events WHERE event_date >= '2026-03-01'").fetchone()[0]
    assert n == 0
