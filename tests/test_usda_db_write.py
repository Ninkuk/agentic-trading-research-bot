...
def test_write_observations_upsert_in_place():
    ...
    db.write_observations(
        conn, "CORN", "ENDING_STOCKS", [{"period": "2025", "value": 1800.0, "unit": "BU"}]
    )
    db.write_observations(
        conn, "CORN", "ENDING_STOCKS", [{"period": "2025", "value": 1875.0, "unit": "BU"}]
    )
...
def test_v_stocks_to_use_ratio():
    conn = _fresh()
    db.write_observations(
        conn, "CORN", "ENDING_STOCKS", [{"period": "2025", "value": 2000.0, "unit": "BU"}]
    )
    db.write_observations(
        conn, "CORN", "TOTAL_USE", [{"period": "2025", "value": 14000.0, "unit": "BU"}]
    )
    row = conn.execute(
        "SELECT ending_stocks, total_use, stocks_to_use FROM v_stocks_to_use WHERE commodity='CORN'"
    ).fetchone()
...
def test_v_stocks_to_use_null_when_total_use_absent():
    conn = _fresh()
    db.write_observations(
        conn, "WHEAT", "ENDING_STOCKS", [{"period": "2025", "value": 800.0, "unit": "BU"}]
    )
    row = conn.execute(
        "SELECT total_use, stocks_to_use FROM v_stocks_to_use WHERE commodity='WHEAT'"
    ).fetchone()
...
def test_prune_snapshots_not_obs():
    conn = _fresh()
    db.write_observations(
        conn, "CORN", "PRODUCTION", [{"period": "2025", "value": 15000.0, "unit": "BU"}]
    )
