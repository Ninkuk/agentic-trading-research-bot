            ...
            "options": [
                {
                    ...
                    "vega": 0.03,
                    "rho": 0.07,
                    "open_interest": 3131.0,
                    "volume": 40.0,
                    "last_trade_price": 99.0,
                    "theo": 99.08,
                },
                {
                    "option": "AAPL260717C00300000",
                    "bid": 15.0,
                    "ask": 15.5,
                    "iv": 0.30,
                    "delta": 0.55,
                    ...
                    "open_interest": 0.0,
                    "volume": 500.0,
                    "last_trade_price": 15.2,
                    "theo": 15.25,
                },
                {
                    "option": "AAPL260717P00300000",
                    "bid": 6.0,
                    "ask": 6.4,
                    "iv": 0.28,
                    "delta": -0.45,
...
def test_parse_occ_call_and_put():
    assert fetch.parse_occ("AAPL260717C00210000") == ("AAPL", "2026-07-17", "call", 210.0)
    assert fetch.parse_occ("AAPL260717P00300000") == ("AAPL", "2026-07-17", "put", 300.0)
...
def test_parse_occ_index_and_adjusted_roots():
    ...
    assert fetch.parse_occ("AAPL1260717C00007500") == ("AAPL1", "2026-07-17", "call", 7.5)
...
def _index_payload():
    ...
    return {
        ...
        "data": {
            "symbol": "SPX",
            "current_price": 5010.5,
            "close": 5005.0,
            "iv30": 14.2,
            "last_trade_time": "2026-07-02T16:14:59",
            "options": [
                {
                    "option": "SPXW260320C05000000",
                    "bid": 120.0,
                    "ask": 122.0,
                    "iv": 0.15,
                    "delta": 0.55,
...
def test_fetch_chain_parses_json():
    ...
    assert fetch.fetch_chain("AAPL", False, get=lambda url, opener=None: body)["symbol"] == "AAPL"
...
def test_http_get_retries_then_raises_non_retryable():
    ...
    with pytest.raises(urllib.error.HTTPError):
        fetch._http_get("u", opener=opener, attempts=3, base_delay=0.1, sleep=slept.append)
