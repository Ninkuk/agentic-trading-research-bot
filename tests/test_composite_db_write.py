...
def test_apply_crosswalk_direct_row_wins(tmp_path):
    ...
    sid = db.write_snapshot(conn, NOW, 1)
    db.write_signal_values(conn, sid, [
        _row(signal_id="cftc_mm_extreme", grain="ticker", entity="XLE",
             score=-1, via_crosswalk=0),
        _row(signal_id="cftc_mm_extreme", grain="asset_class",
             entity="energy", score=2),
    ])
    n = db.apply_crosswalk(conn, sid, {"energy": ["XLE", "XOM"]})
    assert n == 1
    got = {r[0]: r[1:] for r in conn.execute(
        "SELECT entity, score, via_crosswalk FROM signal_values"
        " WHERE snapshot_id=? AND grain='ticker'", (sid,))}
    assert got["XLE"] == (-1, 0)
    assert got["XOM"] == (2, 1)
...
def test_write_market_regime_risk_off(tmp_path):
    ...
    sid = db.write_snapshot(conn, NOW, 1)
    db.write_signal_values(conn, sid, [
        _row(signal_id="cboe_vix", grain="market", entity="*",
             raw_value=31.0, score=2),
        _row(signal_id="fred_hy_spread", grain="market", entity="*",
             raw_value=5.5, score=2),
    ])
    db.write_market_regime(
        conn, sid, {"cboe_vix": "vix", "fred_hy_spread": "hy_spread"})
    got = conn.execute(
        "SELECT vix, hy_spread, regime, inputs_expected, inputs_present"
        " FROM market_regime WHERE snapshot_id=?", (sid,)).fetchone()
...
def test_write_market_regime_risk_on(tmp_path):
    ...
    db.write_signal_values(conn, sid, [
        _row(signal_id="cboe_vix", grain="market", entity="*",
             raw_value=15.0, score=0),
        _row(signal_id="fred_hy_spread", grain="market", entity="*",
             raw_value=2.5, score=0),
    ])
    db.write_market_regime(
        conn, sid, {"cboe_vix": "vix", "fred_hy_spread": "hy_spread"})
    got = conn.execute(
...
def test_write_market_regime_null_raw_value_not_present(tmp_path):
    ...
    db.write_signal_values(conn, sid, [
        _row(signal_id="cboe_vix", grain="market", entity="*",
             raw_value=None, score=0),
    ])
    db.write_market_regime(conn, sid, {"cboe_vix": "vix"})
    got = conn.execute(
        "SELECT vix, inputs_present FROM market_regime"
