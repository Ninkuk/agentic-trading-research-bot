...
def test_run_shifted_date_updates_in_place_no_duplicate(tmp_path):
    ...
    runmod.run(db_path, fetch_forward=lambda: [_row("AAPL", "2026-07-08")], now_iso=NOW)
    runmod.run(
        db_path, fetch_forward=lambda: [_row("AAPL", "2026-07-09")], now_iso=NOW
    )  # date moved
    conn = sqlite3.connect(db_path)
    dates = [r[0] for r in conn.execute("SELECT event_date FROM events WHERE subtype='AAPL'")]
...
def test_run_edgar_confirm_flips_status_and_source(tmp_path):
    ...
    conn = sqlite3.connect(db_path)
    row = conn.execute("SELECT status, source FROM events WHERE subtype='AAPL'").fetchone()
