...
MAP = {1: {"ticker": "AAA", "title": "Alpha"}, 2: {"ticker": "BBB", "title": "Beta"}}
...
def test_run_frames_path_writes_companies_and_facts(tmp_path):
    ...
    sid, ncomp, nfact = runmod.run(
        db_path, only=["Assets"], fetch_frame=fetch_frame, fetch_map=lambda: MAP, now_iso=NOW
    )
    ...
    assert forms == {"FRAME"}
    assert conn.execute("SELECT ticker FROM companies WHERE cik=1").fetchone() == ("AAA",)
...
def test_run_instant_concept_requests_I_suffixed_period():
    ...
    runmod.run(
        ":memory:", only=["Assets"], fetch_frame=fetch_frame, fetch_map=lambda: {}, now_iso=NOW
    )
...
def test_run_bulk_enumerates_quarters_skips_unpublished_labels_from_sub(tmp_path):
    ...
    assert conn.execute("SELECT form, value FROM facts WHERE cik=320193").fetchone() == (
        "10-K",
        100.0,
    )
