...
def _row(commodity, region, metric, my, value, unit):
    return {"commodity": commodity, "region": region, "metric": metric,
            "market_year": my, "value": value, "unit": unit,
...
def test_write_wasde_keeps_unit_bases_distinct_and_upserts(tmp_path):
    ...
    db.write_wasde(conn, [
        _row("Corn", "United States", "ending_stocks", "2025/26", 2100.0, "Million Bushels")])
    assert conn.execute("SELECT COUNT(*) FROM wasde_obs").fetchone()[0] == 2
    assert conn.execute(
        "SELECT value FROM wasde_obs WHERE unit='Million Bushels'").fetchone()[0] == 2100.0
...
def test_v_wasde_stocks_to_use_uses_total_use(tmp_path):
    ...
    r = conn.execute(
        ...
        "WHERE commodity='Corn' AND unit='Million Bushels'").fetchone()
...
def test_v_wasde_stocks_to_use_falls_back_to_domestic_plus_exports(tmp_path):
    ...
    r = conn.execute(
        "SELECT total_use, stocks_to_use FROM v_wasde_stocks_to_use "
        "WHERE commodity='Soybeans'").fetchone()
