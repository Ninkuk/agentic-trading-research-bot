from sources.screeners.edgar_screener.db import (
    connect,
    ensure_schema,
    prune,
    upsert_issuers,
    ...
    return [
        {
            ...
            "path": "edgar/data/1000623/0001-25-001.txt",
        },
        {
            "accession": "0002-25-002",
            "cik": 1000623,
            "company": "Mativ",
            "ticker": "MATV",
            "form": "4",
            "bucket": "insider",
            "filed_date": "2025-06-02",
            "path": "edgar/data/1000623/0002-25-002.txt",
        },
        {
            "accession": "0003-25-003",
            "cik": 999999,
            "company": "Private Co",
            "ticker": None,
            "form": "D",
            "bucket": "other",
            "filed_date": "2025-06-02",
            "path": "edgar/data/999999/0003-25-003.txt",
        },
    ]
...
def test_write_snapshot_stores_rows_and_count():
    ...
    assert n == 3
    assert conn.execute("SELECT filing_count FROM snapshots WHERE id=?", (sid,)).fetchone()[0] == 3
    assert (
        conn.execute(
            "SELECT COUNT(*) FROM filings WHERE snapshot_id=? AND cik=1000623", (sid,)
        ).fetchone()[0]
        == 2
    )
...
def test_v_insider_activity_counts_clusters():
    ...
    row = conn.execute(
        "SELECT insider_filings FROM v_insider_activity WHERE ticker='MATV'"
    ).fetchone()
    assert row[0] == 2
...
def test_upsert_issuers_preserves_first_seen():
    ...
    row = conn.execute(
        "SELECT ticker, first_seen, last_seen FROM issuers WHERE cik=1000623"
    ).fetchone()
        ...
        [
            {
                ...
                "path": "edgar/data/1000623/0009-25-009.txt",
            }
        ],
    )
    delta = conn.execute(
        "SELECT filings_delta_since_last FROM v_activity_history "
        "WHERE ticker='MATV' AND index_date='2025-06-03'"
    ).fetchone()[0]
    assert delta == -1  # 1 - 2
...
def test_v_activity_history_uses_latest_snapshot_per_index_date():
    ...
    count = conn.execute(
        ...
        "WHERE ticker='MATV' AND index_date='2025-06-02'"
    ).fetchone()[0]
...
def test_write_snapshot_dedupes_identical_index_lines():
    ...
    rows = [
        {
            ...
            "bucket": "other",
            "filed_date": "2025-06-02",
            "path": "edgar/data/1000623/0005-25-005.txt",
        },
        {
            "accession": "0005-25-005",
            "cik": 1000623,
        ...
        },
        {
            ...
            "bucket": "other",
            "filed_date": "2025-06-02",
            "path": "edgar/data/999999/0006-25-006.txt",
        },
    ]
    ...
    assert (
        conn.execute(
            ...
            (sid, "0005-25-005", 1000623),
        ).fetchone()[0]
        == 1
    )
    assert conn.execute("SELECT filing_count FROM snapshots WHERE id=?", (sid,)).fetchone()[0] == 2
