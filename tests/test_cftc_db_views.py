...
def test_v_net_computes_net_positions():
    ...
    net = conn.execute("SELECT net_noncomm FROM v_net WHERE code='G'").fetchone()[0]
    assert net == 217028 - 35689


def test_v_latest_picks_most_recent_week():
    conn = _fresh()
    _seed(conn, "G", [("2026-06-09", 10, 0), ("2026-06-23", 50, 0), ("2026-06-16", 20, 0)])
    row = conn.execute("SELECT report_date, net_noncomm FROM v_latest WHERE code='G'").fetchone()
...
def test_cot_index_is_percentile_within_range():
    ...
    _seed(conn, "G", [("2026-06-09", 0, 0), ("2026-06-16", 100, 0), ("2026-06-23", 50, 0)])
    idx = conn.execute("SELECT cot_index FROM v_cot_index_latest WHERE code='G'").fetchone()[0]
...
def test_v_extremes_flags_crowded_market_only():
    ...
    _seed(
        conn, "MILD", [("2026-06-16", 0, 0), ("2026-06-23", 100, 0), ("2026-06-30", 50, 0)]
    )  # index 50
...
def test_cot_index_is_null_on_flat_window():
    ...
    idx = conn.execute("SELECT cot_index FROM v_cot_index_latest WHERE code='FLAT'").fetchone()[0]
    assert idx is None
...
def test_v_extremes_flags_crowded_short():
    ...
    idx = conn.execute("SELECT cot_index FROM v_cot_index_latest WHERE code='SHORT'").fetchone()[0]
    assert idx == 0.0
    codes = {r[0] for r in conn.execute("SELECT code FROM v_extremes")}
...
def test_v_positioning_passthrough_columns():
    conn = _fresh()
    db.upsert_markets(
        conn, [{"code": "P", "name": "Pork", "asset_class": "ags"}], "2026-07-03T00:00:00+00:00"
    )
    ...
    row = conn.execute(
        ...
        "FROM v_positioning WHERE code='P'"
    ).fetchone()
...
def _seed_disagg_mm(conn, code, series, asset_class="metals"):
    ...
    db.upsert_markets(
        conn, [{"code": code, "name": "M", "asset_class": asset_class}], "2026-07-03T00:00:00+00:00"
    )
    rows = [
        {"code": code, "report_date": d, "mm_long": lo, "mm_short": sh, "open_interest": 1000}
        for (d, lo, sh) in series
    ]
...
def _seed_tff_lev(conn, code, series, asset_class="equity_index"):
    ...
    rows = [
        {"code": code, "report_date": d, "lev_long": lo, "lev_short": sh, "open_interest": 1000}
        for (d, lo, sh) in series
    ]
...
def _seed_disagg(conn, code, series, asset_class="metals"):
    ...
    db.upsert_markets(
        conn, [{"code": code, "name": "M", "asset_class": asset_class}], "2026-07-04T00:00:00+00:00"
    )
    rows = [
        {
            "code": code,
...
def test_disagg_cot_index_is_percentile_of_managed_money_net():
    ...
    _seed_disagg_mm(
        conn, "G", [("2026-06-09", 0, 0), ("2026-06-16", 100, 0), ("2026-06-23", 50, 0)]
    )
    idx = conn.execute("SELECT cot_index FROM v_disagg_cot_index_latest WHERE code='G'").fetchone()[
        0
    ]
    assert abs(idx - 50.0) < 1e-9
...
def test_managed_money_extremes_flags_crowded_only():
    ...
    codes = {r[0] for r in conn.execute("SELECT code FROM v_managed_money_extremes")}
...
def test_tff_cot_index_is_100_at_leveraged_max():
    ...
    idx = conn.execute("SELECT cot_index FROM v_tff_cot_index_latest WHERE code='S'").fetchone()[0]
    assert idx == 100.0
...
def test_leveraged_funds_extremes_flags_crowded_short():
    ...
    codes = {r[0] for r in conn.execute("SELECT code FROM v_leveraged_funds_extremes")}
    assert "SH" in codes
...
def test_commercial_index_keys_on_net_prod_merc():
    ...
    _seed_disagg(conn, "G", [("2026-06-09", 0, 0), ("2026-06-16", 100, 0), ("2026-06-23", 50, 0)])
    row = conn.execute(
        "SELECT report_date, net_prod_merc, cot_index "
        "FROM v_disagg_cot_index_commercial_latest WHERE code='G'"
    ).fetchone()
...
def test_commercial_index_null_when_range_degenerate():
    ...
    idx = conn.execute(
        "SELECT cot_index FROM v_disagg_cot_index_commercial_latest WHERE code='G'"
    ).fetchone()[0]
    assert idx is None
...
def test_dealer_index_keys_on_net_dealer():
    ...
    row = conn.execute(
        ...
        "FROM v_tff_cot_index_dealer_latest WHERE code='T'"
    ).fetchone()
...
def test_speculator_views_unchanged_by_commercial_twins():
    ...
    row = conn.execute(
        "SELECT net_mm, cot_index FROM v_disagg_cot_index_latest WHERE code='G'"
    ).fetchone()
