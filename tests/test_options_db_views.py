...
def _contract_row(underlying, snapshot_date, strike):
    return {"occ_symbol": f"{underlying}{snapshot_date[2:4]}{snapshot_date[5:7]}"
                          f"{snapshot_date[8:10]}C{int(strike * 1000):08d}",
            "underlying": underlying, "expiration": "2026-08-21",
            ...
            "underlying_price": 100.0, "vol_oi_ratio": 2.5}


def _daily_row(underlying, iv30):
    return {"underlying": underlying, "underlying_price": 100.0, "close": 100.5,
            "iv30": iv30, "total_call_volume": 250, "total_put_volume": 100,
...
def test_v_latest_sentiment_is_per_underlying_latest_not_global_max():
    ...
    rows = conn.execute(
        "SELECT underlying, snapshot_date FROM v_latest_sentiment "
...
def test_v_unusual_activity_includes_both_underlyings_own_latest_day():
    conn = db.connect(":memory:"); db.ensure_schema(conn); _seed(conn)
    rows = conn.execute(
        "SELECT underlying, snapshot_date FROM v_unusual_activity "
        "ORDER BY underlying").fetchall()
    assert rows == [("AAA", "2026-07-02"), ("BBB", "2026-07-03")]
...
def test_v_iv_rank_includes_both_underlyings_own_latest_day():
    ...
    rows = {r[0]: r for r in conn.execute(
        "SELECT underlying, snapshot_date, iv30, iv_rank FROM v_iv_rank")}
