...
def _rows(underlying="AAPL"):
    return [
        {
            "occ_symbol": f"{underlying}260717C00210000",
...
def test_upsert_underlying_extends_first_last_seen():
    ...
    row = conn.execute("SELECT first_seen, last_seen, is_index FROM underlyings").fetchone()
...
def test_replace_day_writes_columns_and_source():
    ...
    r = conn.execute(
        "SELECT source, fetched_at, iv, open_interest, vol_oi_ratio FROM option_snapshots"
    ).fetchone()
...
def test_replace_day_isolated_per_underlying():
    ...
    db.replace_day(conn, "2026-07-02", "AAPL", _rows("AAPL"), FA)
    n = conn.execute("SELECT COUNT(*) FROM option_snapshots WHERE underlying='MSFT'").fetchone()[0]
    assert n == 1
