...
def test_write_observations_stores_none_as_null():
    ...
    got = conn.execute("SELECT value FROM observations WHERE series_id='X'").fetchone()
...
def test_v_latest_picks_latest_non_null():
    ...
    row = conn.execute("SELECT date, value FROM v_latest WHERE series_id='X'").fetchone()
...
def test_v_zscore_sign_on_known_distribution():
    ...
    z = conn.execute("SELECT zscore FROM v_zscore WHERE series_id='X'").fetchone()[0]
    assert z > 0
...
def test_v_yoy_change_computes_delta():
    ...
    row = conn.execute(
        "SELECT latest, year_ago, change_abs, change_pct FROM v_yoy_change WHERE series_id='X'"
    ).fetchone()
    assert row[0] == 110.0
    assert row[1] == 100.0
    assert row[2] == 10.0
