...
def test_run_end_to_end_counts_and_snapshots(tmp_path):
    ...
    sid, count = runmod.run(
        db_path,
        fetch_forward=lambda: [_row("A", "2026-07-08"), _row("B", "2026-07-09")],
        now_iso=NOW,
    )
    assert count == 2
...
def test_run_transient_feed_failure_preserves_calendar_hides_secret(tmp_path, capsys):
    db_path = str(tmp_path / "e.db")
    runmod.run(db_path, fetch_forward=lambda: [_row("A", "2026-07-08")], now_iso=NOW)
...
def test_run_estimates_missing_watched_ticker_via_edgar_cadence(tmp_path):
    ...
    conn = sqlite3.connect(db_path)
    row = conn.execute("SELECT event_date, status, source FROM events WHERE subtype='Z'").fetchone()
    assert row == ("2026-10-15", "scheduled", "edgar-estimate")
    # the covered ticker is unaffected and stays the aggregator forward date
    assert conn.execute("SELECT source FROM events WHERE subtype='A'").fetchone() == (
        "stockanalysis",
    )
...
def test_run_estimate_respects_horizon(tmp_path):
    ...
    runmod.run(
        db_path,
        only=["A", "Z"],
        horizon_days=30,  # cutoff ~2026-08-05
        fetch_forward=lambda: [_row("A", "2026-07-08")],
        confirm=lambda *a, **k: set(),
        history=lambda tickers: {t: hist[t] for t in tickers if t in hist},
        now_iso=NOW,
    ...
    assert conn.execute("SELECT COUNT(*) FROM events WHERE subtype='Z'").fetchone()[0] == 0
