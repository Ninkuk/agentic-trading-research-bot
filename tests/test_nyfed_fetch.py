...
def test_build_url_encodes_dates():
    url = fetch._build_url(
        "/rates/all/search.json", {"startDate": "2026-01-01", "endDate": "2026-02-01"}
    )
    assert "startDate=2026-01-01" in url and "endDate=2026-02-01" in url


def test_fetch_domain_extracts_records_from_envelope():
    payload = {"refRates": [{"effectiveDate": "2026-06-01", "type": "SOFR", "percentRate": "5.31"}]}
    ...
    records = [
        {
            ...
            "totalAmtAccepted": "90",
            "details": [{"amtAccepted": 90, "percentOfferingRate": 4.25, "percentAwardRate": 4.25}],
        },
        {
            "operationId": "RP2",
            "operationDate": "2026-06-01",
