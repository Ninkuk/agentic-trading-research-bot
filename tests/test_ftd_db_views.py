...
def test_v_persistent_active_streak_of_six():
    ...
    row = conn.execute("SELECT streak_days, active FROM v_persistent WHERE cusip='A'").fetchone()
...
def test_v_fail_streaks_below_threshold_day_splits_streak():
    ...
    streaks = sorted(
        r[0] for r in conn.execute("SELECT streak_days FROM v_fail_streaks WHERE cusip='A'")
    )
    assert streaks == [3, 3]
    assert conn.execute("SELECT COUNT(*) FROM v_persistent WHERE cusip='A'").fetchone()[0] == 0
...
def test_v_spikes_ratio_against_trailing_average():
    ...
    q, base, ratio = conn.execute(
        "SELECT quantity, base, spike_ratio FROM v_spikes WHERE cusip='A'"
    ).fetchone()
    assert q == 5000
    assert base == 1000.0
    assert ratio == 5.0
...
def test_v_latest_fails_returns_only_max_date():
    ...
    got = {r[0]: r[1] for r in conn.execute("SELECT cusip, quantity FROM v_latest_fails")}
