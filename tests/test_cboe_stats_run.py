...
def _pcr(d):
    return [
        {"date": d, "total_pcr": 0.9, "equity_pcr": 0.7, "index_pcr": None, "total_volume": None}
    ]
...
def test_run_default_fetches_pcr_feed(tmp_path):
    ...
    runmod.run(
        str(tmp_path / "c.db"),
        fetch_pcr=fpcr,
        fetch_vix=lambda fid: _vix("2026-06-01", 14.6),
        now_iso=NOW,
    )
...
def test_run_skips_failing_feed_hides_secret(tmp_path, capsys):
    ...
    sid, fc, rc = runmod.run(
        str(tmp_path / "c.db"), only=["VIX", "VIX3M"], fetch_vix=fetch_vix, now_iso=NOW
    )
...
def test_run_none_feed_skipped(tmp_path):
    sid, fc, rc = runmod.run(
        str(tmp_path / "c.db"), only=["VIX"], fetch_vix=lambda fid: None, now_iso=NOW
    )
...
def test_run_keep_days_prunes_snapshots_not_facts(tmp_path):
    ...
    runmod.run(
        db_path, only=["PCR"], fetch_pcr=lambda: _pcr("2026-06-01"), now_iso=NOW, keep_days=30
    )
    conn = sqlite3.connect(db_path)
    assert conn.execute("SELECT COUNT(*) FROM snapshots").fetchone()[0] == 1
