...
from sources.screeners.finra_short_interest import run as run_mod
...
def test_settlement_dates_bounds_are_inclusive_and_clipped():
    ...
    assert run_mod.settlement_dates("2024-06-20", "2024-07-16") == ["2024-06-28", "2024-07-15"]
...
def test_run_ingests_published_and_skips_unpublished(tmp_path):
    ...
    _, sc, rc = run_mod.run(dbp, start="2024-06-01", now_iso=NOW, fetch_settlement=fetch_settlement)
    assert (sc, rc) == (1, 1)
    conn = db.connect(dbp)
    assert conn.execute("SELECT COUNT(*) FROM short_interest").fetchone()[0] == 1
...
def test_run_incremental_skips_stored_refetches_last_two(tmp_path):
    ...
    dbp = str(tmp_path / "si.db")
    first = []
    run_mod.run(dbp, start="2024-06-01", now_iso=NOW, fetch_settlement=make_fs(first))
    assert len(first) == 5

    second = []
    run_mod.run(dbp, start="2024-06-01", now_iso=NOW, fetch_settlement=make_fs(second))
...
def test_run_full_refetches_every_settlement(tmp_path):
    ...
    second = []
    run_mod.run(dbp, start="2024-06-01", full=True, now_iso=NOW, fetch_settlement=make_fs(second))
    assert len(second) == 5
...
def test_run_all_unpublished_writes_zero_snapshot(tmp_path):
    ...
    assert (sc, rc) == (0, 0)
    conn = db.connect(dbp)
    assert tuple(conn.execute("SELECT settlement_count, row_count FROM snapshots").fetchone()) == (
        0,
        0,
    )
...
def test_run_keep_days_prunes_old_snapshots(tmp_path):
    ...
    conn = db.connect(dbp)
    assert (
        conn.execute("SELECT COUNT(*) FROM snapshots WHERE captured_at < '2020-01-01'").fetchone()[
            0
        ]
        == 0
    )
