...
def test_run_is_idempotent_no_duplicates(tmp_path):
    ...
    dupes = conn.execute(
        ...
        "GROUP BY event_type, event_date HAVING c > 1"
    ).fetchall()
    assert dupes == []
...
def test_run_refresh_merges_parsed_pages_over_seed(tmp_path):
    db_path = str(tmp_path / "m.db")
    pages = {
        "nyse": "<tr><th>Holiday</th><th>2026</th></tr>"
    ...
    conn = sqlite3.connect(db_path)
    hit = conn.execute(
        "SELECT title FROM events WHERE event_date='2026-06-15' AND event_type='market_holiday'"
    ).fetchone()
