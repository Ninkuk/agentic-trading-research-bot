...
def _evt(ticker, event_date, status="scheduled", source="stockanalysis", mktcap=1e9, timing="amc"):
    return {
        "event_type": "earnings",
        "event_date": event_date,
        "event_time": "after close",
        "subtype": ticker,
        "title": ticker,
        "status": status,
        "source": source,
...
def test_v_upcoming_earnings_filters_future_orders_by_date_then_mktcap():
    ...
    rows = [r[0] for r in conn.execute("SELECT ticker FROM v_upcoming_earnings")]
    assert rows == ["BIG", "SM"]
...
def test_v_imminent_earnings_respects_horizon():
    ...
    rows = [r[0] for r in conn.execute("SELECT ticker FROM v_imminent_earnings")]
    assert rows == ["A"]
...
def test_v_this_week_earnings_mon_to_fri():
    ...
    rows = [r[0] for r in conn.execute("SELECT ticker FROM v_this_week_earnings")]
    assert rows == ["MON", "FRI"]
...
def test_v_earnings_confirmed_only_edgar_verified():
    ...
    rows = [r[0] for r in conn.execute("SELECT ticker FROM v_earnings_confirmed")]
    assert rows == ["B"]
