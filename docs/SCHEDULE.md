...
| `research-nightly` | every day 10:00pm | Headless `claude -p "/research-ticker <T>"` per selected ticker (≤`RESEARCH_NIGHTLY_MAX`, default 3, on `RESEARCH_NIGHTLY_MODEL`, default opus). Selects tonight's new `v_flagged` names first, then stale flagged, then stale held (`RESEARCH_STALE_DAYS`, default 30). Read-only tool allowlist — order tools never granted. Success = fresh `research/<T>-<date>.md` ≥2KB; exits nonzero only if ALL selected fail. Theses sit untracked until human review |
