...
CREATE VIEW IF NOT EXISTS v_gate_input AS
SELECT c.instrument, c.instrument_kind, c.direction, c.det_score,
       ...
       c.details, s.equity, s.regime_scalar, s.config_hash
...
def write_snapshot(conn, captured_at, equity, regime_scalar,
                   leads_snapshot_id, config_hash) -> int:
    cur = conn.execute(
        "INSERT INTO snapshots (captured_at, candidate_count, rejection_count,"
        " equity, regime_scalar, leads_snapshot_id, config_hash)"
        " VALUES (?, 0, 0, ?, ?, ?, ?)",
