...
from sources.screeners.usda_screener import catalog, db, fetch, wasde
...
def run(
    ...
    api_key=None,
    now_iso=None,
    fetch_target=fetch.fetch_target,
):
    ...
    ids = catalog.select_ids([s.id for s in catalog.CATALOG], only, exclude, add=add)
    ...
    try:
        ...
        for cid in ids:
            ...
            try:
                ...
                conn.rollback()
                print(f"warning: skipping {cid}: {type(e).__name__}", file=sys.stderr)
                continue
            successes += 1
            total_obs += n

        if successes == 0:
            print("warning: no USDA targets fetched (0 series, 0 observations)", file=sys.stderr)
...
def run_wasde(db_path, keep_days=None, now_iso=None, fetch=wasde.fetch_wasde):
    """Ingest the WASDE balance sheet into the wasde_obs sibling. The fetch
    discovers the newest published release from the report landing page (only
    the current month is hosted). No API key needed. Returns
    (snapshot_id, commodity_count, observation_count)."""
    now_iso = now_iso or datetime.now(UTC).isoformat()
    ...
    try:
        ...
        try:
            rows = fetch()
        except Exception as e:  # loud but non-fatal (schema drift, network)
            print(f"warning: WASDE: {type(e).__name__}", file=sys.stderr)
            rows = None
        if rows:
            obs_count = db.write_wasde(conn, rows)
            commodities = {r["commodity"] for r in rows}
        else:
            print("warning: no WASDE release found (0 commodities, 0 rows)", file=sys.stderr)
...
def main(argv=None):
    p = argparse.ArgumentParser(
        prog="usda", description="Pull USDA crop supply/demand balance-sheet data into SQLite"
    )
    ...
    p.add_argument(
        "--wasde",
        action="store_true",
        help="ingest the WASDE balance sheet (OCE report XML) "
        "instead of Quick Stats — no API key needed; only the "
        "current release is hosted, so there is no month pin",
    )
    a = p.parse_args(argv)
    if a.wasde:
        _, cc, oc = run_wasde(a.db, keep_days=a.keep_days)
        print(f"stored {oc} WASDE observations across {cc} commodities into {a.db}")
        return
    _, sc, oc = run(
        a.db, only=_split(a.only), exclude=_split(a.exclude), add=a.add, keep_days=a.keep_days
    )
