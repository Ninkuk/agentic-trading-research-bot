...
def run(
    db_path, start=None, keep_days=None, full=False, fetch_day=fetch.fetch_day, now_iso=None
) -> tuple[int, int, int]:
    ...
    try:
        ...
        for day in all_days:
            ...
            try:
                ...
                conn.rollback()
                print(f"warning: skipping {day}: {type(e).__name__}", file=sys.stderr)
                continue
...
def main(argv=None):
    p = argparse.ArgumentParser(
        prog="short_volume", description="Pull FINRA daily short sale volume into SQLite"
    )
    p.add_argument("--db", default="short_volume.db")
    p.add_argument(
        "--start", default=None, help="earliest trading date YYYY-MM-DD (default: ~6 months back)"
    )
    p.add_argument(
        "--full",
        action="store_true",
        help="re-ingest every day in range, ignoring the incremental skip",
    )
    p.add_argument(
        "--keep-days",
        type=int,
        default=None,
        help="prune snapshot provenance older than N days (never touches short-volume history)",
    )
    a = p.parse_args(argv)
