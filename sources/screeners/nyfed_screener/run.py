...
from sources.screeners.nyfed_screener import catalog, db, fetch
...
def _since_floor(last, start, full):
    ...
    if full or last is None:
        return start
    return (datetime.fromisoformat(last) - timedelta(days=_LOOKBACK_DAYS)).date().isoformat()


_DATE_COL = {
    "reference_rates": "effective_date",
    "repo_ops": "operation_date",
    "soma_holdings": "as_of_date",
    "primary_dealer_stats": "as_of_date",
}
_WRITER = {
    "reference_rates": db.write_reference_rates,
    "rrp": db.write_repo_ops,
    "repo": db.write_repo_ops,
    "soma": db.write_soma_holdings,
...
def run(
    ...
    keep_days=None,
    full=False,
    fetch_domain=fetch.fetch_domain,
    now_iso=None,
):
    ...
    try:
        ...
        for domain_id in ids:
            ...
            if ds is None:
                ...
                continue
            try:
                since = _since_floor(_max_date(conn, ds.table, _DATE_COL[ds.table]), start, full)
                records = fetch_domain(ds.endpoint, start=since)
                n = _WRITER[domain_id](conn, _parse(domain_id, records))
            except Exception as e:
                conn.rollback()
                print(f"warning: skipping {domain_id}: {type(e).__name__}", file=sys.stderr)
                continue
            successes += 1
            total_rows += n

        if successes == 0:
            print("warning: no NY Fed domains fetched (0 domains, 0 rows)", file=sys.stderr)
...
def main(argv=None):
    p = argparse.ArgumentParser(
        prog="nyfed", description="Pull NY Fed Markets data (rates/repo/SOMA) into SQLite"
    )
    ...
    p.add_argument("--start", default=None, help="date floor for the first fetch (YYYY-MM-DD)")
    p.add_argument(
        "--full",
        action="store_true",
        help="re-pull from --start (or full history), ignoring the incremental revision-lookback",
    )
    p.add_argument(
