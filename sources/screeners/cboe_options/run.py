...
from sources.screeners.cboe_options import catalog, db, fetch
...
def run(
    db_path, symbols=None, keep_days=None, now_iso=None, fetch_chain=fetch.fetch_chain
) -> tuple[int, int, int]:
    ...
    try:
        ...
        for symbol in symbols:
            try:
                ...
                conn.rollback()
                print(f"warning: skipping {symbol}: {type(e).__name__}", file=sys.stderr)
                continue
...
def main(argv=None):
    p = argparse.ArgumentParser(
        prog="options", description="Pull CBOE delayed-quote option chains into SQLite"
    )
    p.add_argument("--db", default="cboe_options.db")
    p.add_argument(
        "--only", default=None, help="comma-separated symbols to fetch (default: catalog)"
    )
    p.add_argument("--exclude", default=None, help="comma-separated symbols to skip")
    p.add_argument("--add", default=None, help="comma-separated extra symbols to append")
    p.add_argument(
        "--keep-days",
        type=int,
        default=None,
        help="prune snapshot provenance older than N days (never touches option history)",
    )
    a = p.parse_args(argv)
    all_syms = [u.symbol for u in catalog.CATALOG]
    symbols = catalog.select_symbols(all_syms, _split(a.only), _split(a.exclude), _split(a.add))
