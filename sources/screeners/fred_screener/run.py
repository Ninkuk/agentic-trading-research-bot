...
from sources.screeners.fred_screener import catalog, db, fetch
...
def run(
    ...
    keep_days=None,
    api_key=None,
    fetch_series=fetch.fetch_series,
    fetch_obs=fetch.fetch_observations,
    now_iso=None,
    vintages=False,
    fetch_vintages=fetch.fetch_observation_vintages,
):
    """Fetch selected FRED series into SQLite, upserting observation history.
    Optionally fetch ALFRED observation vintages. Returns (snapshot_id, series_count, observation_count)."""
            ...
            try:
                ...
                if vintages:
                    try:
                        ...
                        Exception
                    ) as e:  # skip vintage fetch on failure, but don't fail the series
                        ...
                        conn.rollback()
                        print(
                            f"warning: skipping vintage fetch for {series_id}: {type(e).__name__}",
                            file=sys.stderr,
                        )
                successes += 1
                ...
                conn.rollback()
                print(f"warning: skipping {series_id}: {type(e).__name__}", file=sys.stderr)
                continue

        if successes == 0:
            print(
                "warning: no FRED series fetched successfully; wrote empty snapshot",
                file=sys.stderr,
            )
...
def main(argv=None):
    p = argparse.ArgumentParser(
        prog="fred", description="Pull curated FRED macro series into SQLite"
    )
    p.add_argument("--db", default="fred.db")
    p.add_argument(
        "--only", default=None, help="comma-separated series ids to pull (default: catalog)"
    )
    p.add_argument("--exclude", default=None, help="comma-separated series ids to skip")
    p.add_argument(
        "--add",
        action="append",
        default=None,
    ...
    p.add_argument(
        "--start", default=None, help="observation_start YYYY-MM-DD (default: full history)"
    )
    ...
    p.add_argument(
        ...
        action="store_true",
        default=False,
        help="also fetch ALFRED observation vintage history",
    )
    a = p.parse_args(argv)
