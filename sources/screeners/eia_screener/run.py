...
from sources.screeners.eia_screener import catalog, db, fetch
...
def run(
    ...
    api_key=None,
    now_iso=None,
    fetch_series_obs=fetch.fetch_series_obs,
):
    ...
    for token in add or []:
        ...
        if not route or not facet:
            print(f"warning: bad --add token {token} (want route:facet)", file=sys.stderr)
            continue
        ad_hoc[facet] = catalog.Series(facet, route, facet, facet, "custom")
        add_ids.append(facet)

    ids = catalog.select_ids([s.series_id for s in catalog.CATALOG], only, exclude, add=add_ids)
    ...
    try:
        ...
        for sid in ids:
            ...
            if s is None:
                continue
            try:
                rows, unit = fetch_series_obs(s.route, s.facet, api_key, start=start)
                db.upsert_series(
                    conn,
                    [
                        {
                ...
                conn.rollback()
                print(f"warning: skipping {sid}: {type(e).__name__}", file=sys.stderr)
                continue
            successes += 1
            total_obs += n

        if successes == 0:
            print("warning: no EIA series fetched (0 series, 0 observations)", file=sys.stderr)
...
def main(argv=None):
    p = argparse.ArgumentParser(
        prog="eia", description="Pull weekly EIA energy-inventory series into SQLite"
    )
    ...
    p.add_argument(
        "--add", action="append", default=None, help="ad-hoc series as route:facet (repeatable)"
    )
