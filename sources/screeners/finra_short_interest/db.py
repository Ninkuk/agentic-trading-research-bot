...
def upsert_securities(conn, rows: list[dict]) -> None:
    ...
    params = [
        {"symbol": r["symbol"], "issue": r.get("issue_name"), "d": r["settlement_date"]}
        for r in rows
    ]
    conn.executemany(
...
def replace_settlement(conn, settlement_date: str, rows: list[dict]) -> int:
    ...
    conn.execute("DELETE FROM short_interest WHERE settlement_date = ?", (settlement_date,))
    placeholders = ", ".join(":" + c for c in _SI_COLS)
    params = [{c: r.get(c) for c in _SI_COLS} for r in by_key.values()]
    conn.executemany(
        f"INSERT INTO short_interest ({', '.join(_SI_COLS)}) VALUES ({placeholders})", params
    )
    conn.commit()
    return len(by_key)


def record_settlement(conn, settlement_date: str, fetched_at: str, row_count: int) -> None:
...
def write_snapshot(conn, captured_at: str, settlement_count: int, row_count: int) -> int:
    """Insert one fetch-run header. Returns the snapshot id."""
    cur = conn.execute(
        "INSERT INTO snapshots (captured_at, settlement_count, row_count) VALUES (?, ?, ?)",
        (captured_at, settlement_count, row_count),
    )
    conn.commit()
...
def stored_settlements(conn) -> list:
    ...
    return [
        r[0]
        for r in conn.execute("SELECT settlement_date FROM settlements ORDER BY settlement_date")
    ]
