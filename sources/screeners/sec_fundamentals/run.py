...
def _ingest_bulk(conn, fetch_bulk, tag_set, ticker_map, ciks_touched, now_iso, bulk_start) -> int:
...
def run(
    ...
    bulk_start=None,
    keep_days=None,
    fetch_frame=fetch.fetch_frame,
    fetch_facts=fetch.fetch_company_facts,
    fetch_map=fetch.fetch_ticker_map,
    fetch_bulk=fetch.fetch_bulk,
    now_iso=None,
):
    ...
    tags = catalog.select_ids([c.tag for c in catalog.CATALOG], only, exclude, add=add)
    tag_set = set(tags)
    ...
    try:
        ...
        print(f"warning: ticker map unavailable: {type(e).__name__}", file=sys.stderr)
    cik_by_ticker = {v["ticker"]: k for k, v in ticker_map.items()}

    conn = db.connect(db_path)
    ciks_touched: set[int] = set()
    fact_total = 0
    try:
        ...
        if bulk:
            ...
            fact_total += _ingest_bulk(
                conn, fetch_bulk, tag_set, ticker_map, ciks_touched, now_iso, bulk_start
            )
        else:
            ...
            for concept in concepts:
                for period in periods:
                    try:
                        ...
                        conn.rollback()
                        print(
                            f"warning: skipping frame {concept.tag}@{period}: {type(e).__name__}",
                            file=sys.stderr,
                        )
                        continue
                    for r in rows:
            ...
            for sym in tickers or []:
                ...
                try:
                    ...
                    conn.rollback()
                    print(
                        f"warning: skipping companyfacts {sym}: {type(e).__name__}", file=sys.stderr
                    )
                    continue
                label = ticker_map.get(cik, {})
                name = payload.get("entityName") or label.get("title")
                db.upsert_companies(
                    conn, [{"cik": cik, "ticker": sym, "name": name, "sic": None}], now_iso
                )
        ...
        if company_count == 0 and fact_total == 0:
            print("warning: no fundamentals fetched (0 companies, 0 facts)", file=sys.stderr)
...
def main(argv=None):
    p = argparse.ArgumentParser(
        prog="fundamentals", description="Pull SEC XBRL company fundamentals into a SQLite panel"
    )
    ...
    p.add_argument(
        "--add", action="append", default=None, help="extra tag not in the catalog (repeatable)"
    )
    p.add_argument(
        "--tickers", default=None, help="watchlist for companyfacts depth (comma-separated)"
    )
    p.add_argument(
        "--period",
        action="append",
    ...
    p.add_argument(
        "--bulk", action="store_true", help="backfill from the quarterly ZIP instead of the APIs"
    )
    p.add_argument(
        "--start",
        default=None,
        help="--bulk start quarter e.g. 2015q1 (default: latest completed quarter)",
    )
    p.add_argument(
    ...
    _, ncomp, nfact = run(
        a.db,
        only=_split(a.only),
        exclude=_split(a.exclude),
        add=a.add,
        tickers=_split(a.tickers),
        periods=a.period,
        bulk=a.bulk,
        bulk_start=a.start,
