...
from sources.screeners.cboe_stats import catalog, db, fetch
...
def run(
    ...
    keep_days=None,
    now_iso=None,
    fetch_pcr=fetch.fetch_pcr,
    fetch_vix=fetch.fetch_vix,
):
    ...
    try:
        ...
        for feed_id in ids:
            ...
            try:
                ...
                conn.rollback()
                print(f"warning: skipping {feed_id}: {type(e).__name__}", file=sys.stderr)
                continue
            successes += 1
            total_rows += n

        if successes == 0:
            print("warning: no CBOE feeds fetched (0 feeds, 0 rows)", file=sys.stderr)
...
def main(argv=None):
    p = argparse.ArgumentParser(
        prog="cboe_stats", description="Pull CBOE market-wide put/call + VIX sentiment into SQLite"
    )
    ...
    p.add_argument(
        "--add", action="append", default=None, help="extra vol-index feed id (repeatable)"
    )
    p.add_argument("--start", default=None, help="filter parsed rows to date >= this (YYYY-MM-DD)")
    p.add_argument(
