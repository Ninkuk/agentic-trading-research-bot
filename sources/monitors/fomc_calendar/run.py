...
def build_events(meetings, now_iso) -> list:
    ...
    for m in meetings:
        ...
        if has_sep:
            out.append(
                _evt("fomc_sep", decision, "14:00", decision, "FOMC SEP (dot plot)", status, {})
            )
        out.append(
            _evt(
                "fomc_minutes",
                fetch.minutes_date(m["end_date"]),
                "14:00",
                decision,
                "FOMC Minutes",
                status,
...
def run(
    db_path, horizon_days=None, keep_days=None, fetch_calendar=fetch.fetch_calendar, now_iso=None
):
    ...
    try:
        ...
        try:
            ...
            print(f"warning: FOMC fetch failed: {type(e).__name__}", file=sys.stderr)
            meetings = []
        ...
        if meetings:
            ...
            for et in _EVENT_TYPES:
                monitor_common.replace_forward_window(conn, et, today, by_type.get(et, []), now_iso)

        count = conn.execute(
            "SELECT COUNT(*) FROM events WHERE event_date >= ?", (today,)
        ).fetchone()[0]
        snapshot_id = monitor_common.write_snapshot(conn, now_iso, count, "federalreserve")
...
def main(argv=None):
    p = argparse.ArgumentParser(
        prog="fomc", description="Pull the FOMC forward calendar (meetings/minutes/blackout/SEP)"
    )
