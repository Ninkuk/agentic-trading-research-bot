...
def run(
    ...
    keep_days=None,
    api_key=None,
    fetch_one=fetch.fetch_release_dates,
    now_iso=None,
):
    ...
    try:
        ...
        for release_id in ids:
            try:
                ...
                rows.extend(fetch.parse_release_dates(raw, {release_id: by_id[release_id]}))
            except Exception as e:  # skip-and-continue; never echo str(e)/e.url
                conn.rollback()
                print(f"warning: skipping {release_id}: {type(e).__name__}", file=sys.stderr)
                continue
...
def main(argv=None):
    ...
    p.add_argument(
        "--only", default=None, help="comma-separated release_ids to pull (default: catalog)"
    )
    p.add_argument("--exclude", default=None, help="comma-separated release_ids to skip")
    p.add_argument(
        "--horizon-days", type=int, default=7, help="imminence window for v_imminent_high_impact"
    )
    p.add_argument(
        "--keep-days",
        type=int,
    ...
    _, count = run(
        a.db, only=only, exclude=exclude, horizon_days=a.horizon_days, keep_days=a.keep_days
    )
