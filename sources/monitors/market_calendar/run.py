...
def run(db_path, years=2, horizon_days=7, keep_days=None, refresh=False, pages=None, now_iso=None):
    ...
    if refresh or pages is not None:
        pages = pages or {
            "nyse": fetch.fetch_page(_NYSE_URL),
    ...
    try:
        ...
        for event_type, rows in by_type.items():
            monitor_common.replace_forward_window(conn, event_type, today, rows, now_iso)
        count = conn.execute(
            "SELECT COUNT(*) FROM events WHERE event_date >= ?", (today,)
        ).fetchone()[0]
        snapshot_id = monitor_common.write_snapshot(conn, now_iso, count, "market_calendar")
...
def _opex_title(iso, kind):
    ...
    return f"{month} Quad Witching" if kind == "quad_witching" else f"{month} Monthly OPEX"
...
def main(argv=None):
    ...
    p.add_argument(
        "--years", type=int, default=2, help="years of OPEX/quad-witching to compute forward"
    )
    p.add_argument("--horizon-days", type=int, default=7, help="imminence window for v_imminent")
    p.add_argument(
        "--keep-days",
        type=int,
        default=None,
    ...
    a = p.parse_args(argv)
    _, count = run(
        a.db, years=a.years, horizon_days=a.horizon_days, keep_days=a.keep_days, refresh=a.refresh
    )
